<template>
  ...
  <script setup>
  import { ref } from 'vue';
  import { createFlashcard } from '../../services/apiService';
  ...
  const newFlashcards = async () => {
    try {
      ...
      const response = await createDeck(props.deckId, deckData);
  ...
  const newFlashcardsIA = async () => {
    try {
      ...
      const response = await createFlashcards(props.userId, deckData);
      console.log("Flashcards créé avec succès :", response);
